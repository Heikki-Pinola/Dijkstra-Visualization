...
\node[state] (5) [right =of 3, fill=red!60] {5};
\node[state] (2) [below =of 1, yshift=-10, fill=red!60] {2};
\node[state] (4) [below =of 3, yshift=-10, fill=red!60] {4};
...
\path[->] 
		...
		(3) edge[auto=right, green, line width=1mm]				(4)
		(3) edge[auto=right] node {5}				(4)
		%(3) edge[auto=left, lightgray, line width=1mm]				(5)
		(3) edge[auto=left] node {10}				(5)
		%(4) edge[auto=right, lightgray, line width=1mm]				(5)
		(4) edge[auto=right, green, line width=1mm]				(5)
...
\foreach \i [count=\j from 0] in {0, 9, 5, 11, 16, 20}
...
%\node[state, label=right: 20] (5) at (0,-0.5) {5};
...
Priority queue is empty => algorithm ended.
